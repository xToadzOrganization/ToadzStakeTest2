...
async function unstakeNft(collectionAddress, tokenId) {
    ...
    try {
        ...
        const stakingContract = new ethers.Contract(CONTRACTS.nftStaking, NFT_STAKING_ABI, signer);
        const tx = await stakingContract.unstake(collectionAddress, tokenId, { gasLimit: 14000000 });
        await tx.wait();
    ...
    try {
        ...
        for (const col of stakeableCollections) {
            ...
            for (let i = 0; i < tokens.length; i += BATCH_SIZE) {
                ...
                const tx = await stakingContract.unstakeBatch(col.address, batch, { gasLimit: 14000000 });
                await tx.wait();
...
async function claimStakingRewards() {
    ...
    try {
        ...
        const stakingContract = new ethers.Contract(CONTRACTS.nftStaking, NFT_STAKING_ABI, signer);
        const tx = await stakingContract.claimRewards({ gasLimit: 14000000 });
        await tx.wait();
